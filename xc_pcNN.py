...
class Net:
    def __init__(self):
        super(Net, self).__init__()  # Required for module inheritance (load_state_dict)
        self.hidden = 16
    ...
    def mkmat(self, seed=0, scale=SCALE, scale_c=SCALE_C):
        ...
        self.fc1c = nn.Linear(4, self.hidden)
    ...
    def addnoise(self, scale=SCALE, scale_c=SCALE_C):
        SQRTH=math.sqrt(self.hidden)
        auxfc1 = nn.Linear(2, self.hidden)
        auxfc2 = nn.Linear(self.hidden, self.hidden)
        auxfc3 = nn.Linear(self.hidden, self.hidden)
        auxfc4 = nn.Linear(self.hidden, 1)
        self.w1 = jnp.array(self.fc1.weight.data.numpy(),
                            dtype=np.float32)+jnp.array(auxfc1.weight.data.numpy(),dtype=np.float32)*SQRT2*scale
        self.w2 = jnp.array(self.fc2.weight.data.numpy(),
                            dtype=np.float32)+jnp.array(auxfc2.weight.data.numpy(),dtype=np.float32)*SQRTH*scale
        self.w3 = jnp.array(self.fc3.weight.data.numpy(),
                            dtype=np.float32)+jnp.array(auxfc3.weight.data.numpy(),dtype=np.float32)*SQRTH*scale
        self.w4 = jnp.array(self.fc4.weight.data.numpy(),
                            dtype=np.float32)+jnp.array(auxfc4.weight.data.numpy(),dtype=np.float32)*SQRTH*scale
        self.b1 = jnp.array(self.fc1.bias.data.numpy(), dtype=np.float32)+jnp.array(auxfc1.bias.data.numpy(), dtype=np.float32)*SQRT2*scale
        self.b2 = jnp.array(self.fc2.bias.data.numpy(), dtype=np.float32)+jnp.array(auxfc2.bias.data.numpy(), dtype=np.float32)*SQRTH*scale
        self.b3 = jnp.array(self.fc3.bias.data.numpy(), dtype=np.float32)+jnp.array(auxfc3.bias.data.numpy(), dtype=np.float32)*SQRTH*scale
        self.b4 = jnp.array(self.fc4.bias.data.numpy(), dtype=np.float32)+jnp.array(auxfc4.bias.data.numpy(), dtype=np.float32)*SQRTH*scale
        auxfc1c = nn.Linear(4, self.hidden)
        auxfc2c = nn.Linear(self.hidden, self.hidden)
        auxfc3c = nn.Linear(self.hidden*2, self.hidden)
        auxfc4c = nn.Linear(self.hidden, 1)
        self.w1c = jnp.array(self.fc1c.weight.data.numpy(),
                             dtype=np.float32)+jnp.array(auxfc1c.weight.data.numpy(),dtype=np.float32)*SQRT2*scale_c
        self.w2c = jnp.array(self.fc2c.weight.data.numpy(),
                             dtype=np.float32)+jnp.array(auxfc2c.weight.data.numpy(),dtype=np.float32)*SQRTH*scale_c
        self.w3c = jnp.array(self.fc3c.weight.data.numpy(),
                             dtype=np.float32)+jnp.array(auxfc3c.weight.data.numpy(),dtype=np.float32)*SQRTH*SQRT2*scale_c
        self.w4c = jnp.array(self.fc4c.weight.data.numpy(),
                             dtype=np.float32)+jnp.array(auxfc4c.weight.data.numpy(),dtype=np.float32)*SQRTH*scale_c
        self.b1c = jnp.array(self.fc1c.bias.data.numpy(),
                             dtype=np.float32)+jnp.array(auxfc1c.bias.data.numpy(),dtype=np.float32)*SQRT2*scale_c
        self.b2c = jnp.array(self.fc2c.bias.data.numpy(),
                             dtype=np.float32)+jnp.array(auxfc2c.bias.data.numpy(),dtype=np.float32)*SQRTH*scale_c
        self.b3c = jnp.array(self.fc3c.bias.data.numpy(),
                             dtype=np.float32)+jnp.array(auxfc3c.bias.data.numpy(),dtype=np.float32)*SQRTH*SQRT2*scale_c
        self.b4c = jnp.array(self.fc4c.bias.data.numpy(),
                             dtype=np.float32)+jnp.array(auxfc4c.bias.data.numpy(),dtype=np.float32)*SQRT2*scale_c
    ...
    def calc_c(self, rho, spin):
        if spin != 0:
            #escan, vscan = dft.xcfun.eval_xc(',r2scan', rho, spin=1)[0:2]
            rho1 = rho[0]
            rho2 = rho[1]
            ...
            #escan, vscan = dft.xcfun.eval_xc(',r2scan', rho, spin=0)[0:2]
            rho0, dx, dy, dz, lapl, tau = rho[:6]
            gamma1 = gamma2 = gamma12 = (dx**2+dy**2+dz**2)*0.25
            rho01 = rho02 = rho0*0.5
            tau1 = tau2 = tau*0.5
    ...
    def eval_x(self, n, spin):
        if spin == 0:
            ...
            gr *= df.reshape((-1, 1))
            escan, vscan = dft.xcfun.eval_xc('r2scan,', n, spin=0)[0:2]
            ex = fx*escan
            vlapl = np.zeros(N)
            ...
            escan1, vscan1 = dft.xcfun.eval_xc('r2scan,', n1*2, spin=0)[0:2]
            escan2, vscan2 = dft.xcfun.eval_xc('r2scan,', n2*2, spin=0)[0:2]
            # print(fx1)
    ...
    def eval_c(self, n, spin):
        ...
        escan, vscan = dft.xcfun.eval_xc(',r2scan', n, spin)[0:2]
        ec = fc*escan
        N = escan.shape[0]
        if spin != 0:
            n1, n2 = n
